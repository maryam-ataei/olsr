...
class OLSR_AckTimer : public TimerHandler {
public:
	OLSR_AckTimer(OLSR* agent) : TimerHandler() {
	      agent_ = agent;
	}
protected:
        OLSR*	agent_;
...
class OLSR : public Agent {
	...
	inline int& check() {static int check=0; return(check);}    
	inline int& seeq() {static int seeq=0; return(seeq);}    
	//inline int& nuuuum() {static int nuuuum=0; return(nuuuum);}   
	//int             s=0;
...
protected:
	...
	inline int&		ack_ival()	{ return ack_ival_; }               
	...
	void		forward(OLSR_rt_entry*, Packet* p, double);                          
	void 		recv_ack();                         
	...
	void		send_mid();
	void		send_pkt();
	void		send_msg(nsaddr_t, u_int16_t, u_int16_t);
...
public:
	...
	static int		IsMulticast(nsaddr_t);
};
