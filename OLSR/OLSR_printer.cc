...
#include <fstream>  //addiiiiiiiiiiiiiiing**********************
#include <iostream>  //addiiiiiiiiiiiiiiiiing*******************
#include <olsr/OLSR.h>
...
void
OLSR_printer::print_nbset(FILE* fp, nbset_t& nbset) {

	fp = fopen("nb of nodes.txt",  "app");
	fprintf(fp, "P\tnb\tstatus\twillingness \n");
	//fp->pt_->dump();
	for (nbset_t::iterator it = nbset.begin(); it != nbset.end(); it++) {
		OLSR_nb_tuple* tuple = *it;
		fprintf(fp, "P\t%d\t%d\t%d \n",
