...
/// \brief Receives a ACK message and reschedules the ACK timer.
void
OLSR_AckTimer::expire(Event* e) {
	agent_->recv_ack();
	agent_->set_ack_timer();
...
OLSR_HelloTimer::expire(Event* e) {
  ...
  int a;
	...
	agent_->set_hello_timer();
	 fstream f15 ("/home/maryam/Desktop/karshenasi/new/file/send-hello",ios::in);
			  ...
			  if ( !f15) {
			   ...
			   a=aa+1;
			   fstream f13 ("/home/maryam/Desktop/karshenasi/new/file/send-hello", ios::out);
			  //check for file opening
			  if ( !f13) {
			      cerr << "some thing wrong during opening file!" << endl;
			      exit(1);
...
OLSR_TcTimer::expire(Event* e) {
  ...
  int bb;
	...
	fstream f16 ("/home/maryam/Desktop/karshenasi/new/file/send-tc",ios::in);
			  //check for file opening
			  if ( !f16) {
			      cerr << "some thing wrong during opening file!" << endl;
			      exit(1);
			   ...
			   fstream f14 ("/home/maryam/Desktop/karshenasi/new/file/send-tc", ios::out);
			  //check for file opening
			  if ( !f14) {
			      cerr << "some thing wrong during opening file!" << endl;
			      exit(1);
	...
	// Otherwise, must forward the packet (unless TTL has reached zero)
		...
		if (ih->ttl_ == 0) {
			...
			return;
		      ...
		      int   message_num;
		     double Nxpos,Nypos;
		      MobileNode *qnode = (MobileNode*)Node::get_node_by_address(ra_addr());
		      qnode->update_position();       //update the position, before using it
		      Nxpos = qnode->X();				//node's coordinates
		      Nypos = qnode->Y();				//node's coordinates
		       if ((Nxpos>500)&&(Nxpos<900)&&(Nypos>0)&&(Nypos<100)){
		       debug("%f:we are in destination region\n",CURRENT_TIME);
		       message_num = rh->message_id_;
		       debug("%f:message_id is %d\n", CURRENT_TIME , message_num);
		       int seqq;
		       fstream f6 ("/home/maryam/Desktop/karshenasi/new/ackfile.txt", ios::in);
		       f6 >> seqq;
		       if (rh->message_id_ < seqq){
			debug("%f: old sequence number is %d\n" , CURRENT_TIME , rh->message_id_);
			drop(p);
		       }
		       else {
		       if (nuuuum[ra_addr()] < rh->seqno_) {
			  nuuuum[ra_addr()] = rh->seqno_;
		       if (rtt_id[ra_addr()] < message_num ) {
			  debug("%f:my1seqnr is received\n", CURRENT_TIME);
			 rtt_id[ra_addr()] = message_num;
			 recv_count[ra_addr()]=1; 
			 int hopcount;
			 int seeqqqq;
			 seeqqqq = rh->seqno_;
			    hopcountt = ch->mmcount();
			    debug("%f:hop count is %d\n", CURRENT_TIME , hopcountt);
			    const char* directory = "/home/maryam/Desktop/karshenasi/new/hopcount/";
				const char* fileName = "hopcount";
				char name_buffer[512] ;
				FILE* f = NULL;
				sprintf(name_buffer,"%s%s%d",directory,fileName,ra_addr());
				ofstream f23 (name_buffer, ios::out);
			...
			f23<<hopcountt<<endl;
			f23.close();
			  debug("%f: num for node_id %d is %d\n", CURRENT_TIME , ra_addr() , nuuuum[ra_addr()]);
		      ...
		      debug("%f:messsage broadcast in destination region\n" , CURRENT_TIME);
		       }
		       else if(rtt_id[ra_addr()] == message_num) {
			 int mm;
			 mm = rh->seqno_;
			 int nn;
			 nn = nuuuum[ra_addr()];
			 
		        debug("%f:sequnce number is %d and num is %d and node_id is %d\n" , CURRENT_TIME , mm , nn , ra_addr());
			 recv_count[ra_addr()]=recv_count[ra_addr()]+1;
			  debug("%f:recive count[%d] is %d\n" , CURRENT_TIME , ra_addr() , recv_count[ra_addr()]);
			  
			  int hhopcount;
			  int seqqqq;
			  hhopcount = ch->mmcount();
			  seqqqq = rh->seqno_;
			  const char* directory = "/home/maryam/Desktop/karshenasi/new/hopcount/";
				const char* fileName = "hopcount";
				char name_buffer[512] ;
				FILE* f = NULL;
				sprintf(name_buffer,"%s%s%d",directory,fileName,ra_addr());
				ofstream f24 (name_buffer, ios::app);
			  if ( !f24) {
			      cerr << "some thing wrong during opening file!" << endl;
			      exit(1);
			   }
		      ...
		      ch->direction() = hdr_cmn::DOWN;       //important: change the packet's direction
		      Scheduler::instance().schedule(target_, p, 0.);
		      debug("%f:messsage broadcast in destination again\n" , CURRENT_TIME);
			
			 if (recv_count[ra_addr()] == 10) {
			    if (check() == 1 ) {
				  ofstream f9("/home/maryam/Desktop/karshenasi/new/file/delayinfo",ios::app);;
				  if(!f9) {
				      cerr <<"File1 can not open!"<<endl;
				      exit(1);
				  } 
				  double now	= CURRENT_TIME;
				  f9 <<"r "<<"message_id:"<<message_num<<"  "<<"time:"<<now<<endl;
				  check()=0;
				  }
			   debug("%f:receive count= %d\n", CURRENT_TIME , recv_count[ra_addr()]);
			   int nmessage_id;
			   int finalmessage_id;
			   seeq() = 0;
			   fstream f2 ("/home/maryam/Desktop/karshenasi/new/ackfile.txt", ios::in);
			  //check for file opening
			  if ( !f2) {
			      cerr << "some thing wrong during opening file!" << endl;
			      exit(1);
			   ...
			   finalmessage_id=nmessage_id+1;
			   }
			   
			   fstream f3 ("/home/maryam/Desktop/karshenasi/new/ackfile.txt", ios::out);
			  //check for file opening
			  if ( !f3) {
			      cerr << "some thing wrong during opening file!" << endl;
			      exit(1);
			   ...
			   f3 << finalmessage_id;
			   f3.close();  
			   debug("%f:my1seqnr is %u in reciveeeeeee\n",
				 CURRENT_TIME,
				 finalmessage_id);
			  ...
			  const char* directory = "/home/maryam/Desktop/karshenasi/new/hopcount/";
			   ...
			   char py_cmd[512];
				sprintf(py_cmd,"python hop_count_average.py %s%s%d",directory,fileName,ra_addr());
	...
	for (int i = 0; i < op->count; i++) {
		...
		if (duplicated == NULL) {
			...
			if (msg.msg_type() == OLSR_HELLO_MSG){
				...
				original_address=ra_addr();
				
				const char* directory = "/home/maryam/Desktop/karshenasi/new/hello_source_nodes/";
				const char* fileName = "node";
				const char* fileType = ".txt";
				char name_buffer[512];
				FILE* f = NULL;
				int i;

				sprintf(name_buffer,"%s%s%d%s",directory,fileName,ih->saddr(),fileType);
		//Print to character buffer
				

				//f = fopen(name_buffer,"app");
				
				  MobileNode *qnode = (MobileNode*)Node::get_node_by_address(ra_addr());
				  qnode->update_position();       //update the position, before using it
				  double Nxxpos,Nyypos;
				  Nxxpos = qnode->X();				//node's coordinates
				  Nyypos = qnode->Y();
				
				//fprintf(f, "node id:%f X:%d Y:%d time:%d originator of msg:%f\n",ra_addr(),Nxxpos,Nyypos,CURRENT_TIME, ih->saddr());
				
				//fclose(f);
				
			ofstream f20 (name_buffer, ios::app);
			  //check for file opening
			  if ( !f20) {
			      cerr << "some thing wrong during opening file!" << endl;
			      exit(1);
			   ...
			   f20<<original_address<<","<<Nxxpos<<","<<Nyypos<<","<<CURRENT_TIME<<endl;
		   f20.close();
...
OLSR::forward_default(Packet* p, OLSR_msg& msg, OLSR_dup_tuple* dup_tuple, nsaddr_t local_iface) {
	...
	if (msg.ttl() > 1) {
		...
		if (mprsel_tuple != NULL) {
			...
			retransmitted = true;
			fstream f17 ("/home/maryam/Desktop/karshenasi/new/file/forward-default",ios::in);
			  //check for file opening
			  if ( !f17) {
			      cerr << "some thing wrong during opening file!" << endl;
			      exit(1);
			   ...
			   fstream f12 ("/home/maryam/Desktop/karshenasi/new/file/forward-default", ios::out);
			  //check for file opening
			  if ( !f12) {
			      cerr << "some thing wrong during opening file!" << endl;
			      exit(1);
...
OLSR::forward_data(Packet* p) {
	...
	if (ch->direction() == hdr_cmn::UP &&
		...
		if ((u_int32_t)ih->daddr() == IP_BROADCAST)
			...
			if (entry == NULL) {
		     ...
		     double Nxpos,Nypos;
		      ...
		      Nypos = qnode->Y();
		       if ((Nxpos>500)&&(Nxpos<900)&&(Nypos>0)&&(Nypos<100)) {
			 debug("%f:dont count hop count\n" , CURRENT_TIME);
		       }
		       else {
				ch->mmcount() = ch->mmcount() + 1;
		       }
				if (use_mac()) {
		...
		int source_iid;
	 ifstream f21("/home/maryam/Desktop/karshenasi/new/sourcenode.txt",ios::in);
	  if(!f21) {
	  cerr <<"File1 can not open!"<<endl;
	  exit(1);
         }
		...
		if(ra_addr() == source_iid) {
		  
		fstream f18 ("/home/maryam/Desktop/karshenasi/new/file/forward-data",ios::in);
			  //check for file opening
			  if ( !f18) {
			      cerr << "some thing wrong during opening file!" << endl;
			      exit(1);
			  ...
			  if ((u_int32_t)ih->daddr() != IP_BROADCAST)
			   ...
			   fstream f11 ("/home/maryam/Desktop/karshenasi/new/file/forward-data", ios::out);
			  //check for file opening
			  if ( !f11) {
			      cerr << "some thing wrong during opening file!" << endl;
			      exit(1);
...
OLSR::send_hello() {
	...
	msg.hello().htime()		= OLSR::seconds_to_emf(hello_ival2());
	...
	FILE* fp;
	fp = fopen("nb of nodes.txt",  "app");
	fprintf(fp, "P %f _%d_ Neighbor Set \n",
					CURRENT_TIME,
					OLSR::node_id(ra_addr()));
				//fp->pt_->dump();
				OLSR_printer::print_nbset(fp, nbset());
	fclose(fp);
	
	int source_id;
	 ifstream f7("/home/maryam/Desktop/karshenasi/new/sourcenode.txt",ios::in);
	  if(!f7) {
	  cerr <<"File1 can not open!"<<endl;
	  exit(1);
         }
	 ...
	 const char* directory = "/home/maryam/Desktop/karshenasi/new/hello_source_nodes/";
				const char* fileName = "node";
				const char* fileType = ".txt";
				const int count = source_id+1;
				char name_buffer[512];
				char cmd_buffer[512];
				FILE* f = NULL;
				
				char py_cmd[512];
				sprintf(py_cmd,"python LET.py %s%s%d%s",directory,fileName,ra_addr(),fileType);
		...
		//Print to character buffer
				 ...
				 //system(printf("rm %s%s%d%s",directory,fileName,i,fileType));
				...
				//f = fopen(name_buffer,"app");
				 ...
				 double Nxpos,Nypos;
				MobileNode *qnode = (MobileNode*)Node::get_node_by_address(msg.orig_addr());
				qnode->update_position();       //update the position, before using it
				Nxpos = qnode->X();				//node's coordinates
				Nypos = qnode->Y();
				
				
				ofstream f19 (name_buffer, ios::app);
			  //check for file opening
			  if ( !f19) {
			      cerr << "some thing wrong during opening file!" << endl;
			      exit(1);
			   ...
			   f19<<"###\n" << msg.orig_addr()<<","<<Nxpos<<","<<Nypos<<","<<now<<endl;
	...
	for (int i = 0; i < hello.count; i++) {
		...
		if (nt == OLSR_MPR_NEIGH) {
			...
			for (int j = 0; j < hello_msg.count; j++) {
			  ...
			  ofstream f1 ("/home/maryam/Desktop/karshenasi/new/destination-node.txt", ios::app);;
			  //check for file opening
			  if ( !f1) {
			      cerr << "some thing wrong during opening file!" << endl;
			      exit(1);
			   }
			   //checking destination region
			  if ((Nxpos>500)&&(Nxpos<900)&&(Nypos>0)&&(Nypos<100)){ 
                             //f1.flush();
			    myid = ra_addr();
			    f1 << myid <<endl;
...
OLSR::set_hello_timer() {
	...
	hello_timer_.resched((double)(hello_ival2() - JITTER));
...
OLSR::set_ack_timer()
{
	ack_timer_.resched((double)(ack_ival() - JITTER));
}
...
void
OLSR::set_tc_timer() {
	tc_timer_.resched((double)(tc_ival2() - JITTER));
}
...
/*int 
OLSR::IsMulticast(nsaddr_t dest)
{
  assert(dest != NULL);

  return (ntohl(dest) & 0xF000000) == 0xE000000;
}*/


void
OLSR::send_msg(nsaddr_t ipdst, u_int16_t messageid, u_int16_t msg_seqq) {
	Packet* p		= allocpkt();
	struct hdr_cmn* ch	= HDR_CMN(p);
	struct hdr_ip* ih	= HDR_IP(p);
	struct hdr_rtp*rh	= HDR_RTP(p);
		...
		ch->ptype()		= PT_CBR;
		ch->direction()		= hdr_cmn::DOWN;
		ch->size()		= IP_HDR_LEN + UDP_HDR_LEN + sizeof (hdr_rtp) + 1000 ;
		ch->error()		= 0;
		//ch->next_hop()		= entry->next_addr();
		ch->addr_type()		= NS_AF_INET;
		ch->mmcount()		=0;
		...
		rh->message_id_ 	= messageid;
		rh->seqno_		= msg_seqq;
		
		
		int m;
		int mm;
		fstream f22 ("/home/maryam/Desktop/karshenasi/new/file/send-msg",ios::in);
			  //check for file opening
			  if ( !f22) {
			      cerr << "some thing wrong during opening file!" << endl;
			      exit(1);
			   ...
			   fstream f21 ("/home/maryam/Desktop/karshenasi/new/file/send-msg", ios::out);
			  //check for file opening
			  if ( !f21) {
			      cerr << "some thing wrong during opening file!" << endl;
			      exit(1);
			   ...
			   f21.close(); 
			    ...
			    debug("%f:sequnce number of new message is %d\n", CURRENT_TIME , rh->seqno_);
		forward_data(p);
...
OLSR::recv_ack() {
  ...
  ifstream f7("/home/maryam/Desktop/karshenasi/new/sourcenode.txt",ios::in);
	 ...
	 double now	= CURRENT_TIME;
	  ifstream f5("last_line",ios::in);
		   if(!f5) {
		      cerr <<"File2 can not open!"<<endl;
		      exit(1);
		   }
		   nsaddr_t destaddr;
		   f5>>destaddr;
		   debug("%f:destination node is %d\n", CURRENT_TIME , destaddr);
  
  if (ih->saddr() == source_id) {
    ifstream f4("/home/maryam/Desktop/karshenasi/new/ackfile.txt",ios::in);
	  if(!f4) {
	  cerr <<"File1 can not open!"<<endl;
	  exit(1);
         }
	 ...
	 if(msg_id == 0) {
	   rh->message_id_ = 0;
	   rh->message_id_ = rh->message_id_ + 1;
	   msg_num = rh->message_id_;
	   rh->seqno_ = seeq() + 1;
	   seeq() = rh->seqno_;
	   debug("%f:new message_id is %d and seq_number is %d \n",  CURRENT_TIME , rh->message_id_ , rh->seqno_);
	  
		   
	   send_msg(destaddr, msg_num, rh->seqno_ );
	  // if (s == 0 && msg_num >= msg_id) {
	   if (check() == 0) {
	    ofstream f8("/home/maryam/Desktop/karshenasi/new/file/delayinfo",ios::app);;
	      if(!f8) {
		cerr <<"File1 can not open!"<<endl;
	        exit(1);
	     } 
	    double now	= CURRENT_TIME;
	    f8<<"s "<<"message_id:"<<msg_num<<"  "<<"time:"<<now<<endl;
	    check()=1;
	   }
	 }
	 if(msg_num < msg_id) {
	   ...
	   msg_num = msg_id;
	   rh->message_id_ = msg_num;
	  rh->seqno_ = seeq() + 1;
	   seeq() = rh->seqno_;
	   send_msg(destaddr, rh->message_id_, rh->seqno_);
	   //}
	   debug("%f:new message_id is %d and dest_addrs is %d and seq_number is %d\n",  CURRENT_TIME , rh->message_id_ , destaddr , rh->seqno_);
	   
	   if (check() == 0) {
	    ofstream f10("/home/maryam/Desktop/karshenasi/new/file/delayinfo",ios::app);;
	      if(!f10) {
		cerr <<"File1 can not open!"<<endl;
	        exit(1);
	     } 
	    double now	= CURRENT_TIME;
	    f10<<"s "<<"message_id:"<<msg_num<<"  "<<"time:"<<now<<endl;
	    check()=1;
	   } 
	 }
	 if(msg_num == msg_id) {
	  ...
	  debug("%f: message_id is %d and dest_addrs is %d and seq_number is %d\n", CURRENT_TIME , rh->message_id_ , destaddr , rh->seqno_);
  }
...
OLSR::hello_ival2() {
  ...
  const char* directory = "/home/maryam/Desktop/karshenasi/new/HELLO_INT/";
				const char* fileName = "hello_interval";
			   ...
			   fstream f21 (name_buffer, ios::in);
			    if ( !f21) {
			      cerr << "some thing wrong during opening file!" << endl;
			      exit(1);
			   } 
			 f21 >> hello_int;	 
			   ...
			   else if ((hello_int >= 0) && (hello_int < 100) ) {
			     hello_ival_2 = hello_int * 0.2;
			      if (hello_ival_2 <= 2) {
				hello_ival_2 = 2;
			      }
			 }
...
OLSR::tc_ival2() {
  ...
  const char* directory = "/home/maryam/Desktop/karshenasi/new/HELLO_INT/";
			 ...
			 f21 >> tc_int;
			   ...
			   else if ((tc_int >= 0) && (tc_int < 50) ) {
			     tc_ival_2 = tc_int * 0.6;
			      if (tc_ival_2 <= 5) {
				tc_ival_2 = 5;
			      }
			 }
